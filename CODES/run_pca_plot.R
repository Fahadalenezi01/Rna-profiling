...
pca_plot <- ggplot(pca_data, aes(x = PC1, y = PC2, color = condition, shape = condition)) +
  ...
  # Set custom colors
  scale_color_manual(values = c("Saliva" = "blue", "Blood" = "red")) +
  # Add titles and axis labels with variance explained
  labs(
    title = "PCA of Saliva and Blood Samples",
...
# --- Save the Plot ---
pca_plot_file <- file.path(output_dir, "saliva_vs_blood_PCA_plot.tiff")
